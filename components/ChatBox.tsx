...
import { generateGroundedAnswer } from '../services/ragService';
...
interface ChatBoxProps {
    ...
    toolId: string;
    /** Optional level to include in the prompt (defaults to 'AS Level') */
    level?: string;
}

export const ChatBox: React.FC<ChatBoxProps> = ({ toolId, level = 'AS Level' }) => {
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            const answer = await generateGroundedAnswer(userMsg, level);
            setMessages(prev => [...prev, { role: 'assistant', content: answer }]);
        } catch (e) {
            console.error(e);
            setMessages(prev => [...prev, { role: 'assistant', content: 'Error generating response. Please check your API key.' }]);
        } finally {
